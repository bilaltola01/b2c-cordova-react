...
let createHandlers = ctx => {
  let onItemClick = (item, companyId) => {
    const menuCategory =
      ctx.props.categories && ctx.props.categories.length > 0
        ? ctx.props.categories[item.index]
        : null;
  ...
  let initNavigation = el => {
    ...
    ctx.nav.on('staticClick', (e, pointer, el, index) => {
      ...
      DomUtils.removeClass(
        document.querySelector('.subnav--item.active'),
        'active',
      );
      DomUtils.toggleClass(
        document.querySelectorAll('.subnav--item')[index],
        'active',
      );
  ...
  render() {
    ...
    return (
      <div
        ...
        className="subnav navigation-scroll-container"
      >
        <nav>
